...
/**
 * given an array of items, picks one at random. Useful for multple possible replies to a command.
 * @param {array} array
 * @returns {arrayItem} arrayItem at random
 */
...
destroy: Delete an open tournament if one exists\`\`\``, {
                ...
                if (!res.length) {
                    ...
                    api_util.addPlayers(res[0].tournament.id, message.command.users)
                        .then(done => {
                            ...
                            this.postMessageToChannel(message.channel.name, `Okey dokey, I added ${userStr}.`, {
                                as_user: true
                            });
                        })
                        .catch((response, body) => {
                    ...
                    api_util.getParticipants(res[0].tournament.id)
                        .then(result => {
                            ...
                            var participants = participantArray.length ? "```" + participantArray.sort().join('\n') + "```" : "_none yet_";
                                ...
                                // -- or take a webshot and upload it via form/multipart? websocket?
    ...
    destroy: function(message) {
        api_util.deleteTourney()
            ...
            .catch(err => {
                this.postMessageToChannel(message.channel.name, `there wasn't an open tournament man`, {
                    as_user: true
                });
            })
    },
    ...
    tree: function(message) {
      ...
      api_util.getCurrentTourney()
        .then(response => {
          api_util.getParticipants(response[0].tournament.id)
            .then(participants => {
              ...
              if(!bracket_image) {
                this.postMessageToChannel(message.channel.name, "i need at least two people to make a bracket bro", {as_user: true})
              } else {
                ...
                this.postMessageToChannel(message.channel.name, "ok, just a sec...", {as_user: true})
        ...
        .catch(err => {
          console.log(err);
          this.postMessageToChannel(message.channel.name, `but there's no tournament open yet :(`, {as_user: true});
        })
    ...
    shuffle: function(message) {
      api_util.getCurrentTourney()
        ...
        .catch(err => {
          console.log(err);
          this.postMessageToChannel(message.channel.name, 'shuffle what? There\'s no open tourney', {as_user: true});
        })

        var shufflePlayers = (tournamentId) => {
          api_util.shufflePlayers(tournamentId)
            .then(this.postMessageToChannel(message.channel.name, 'okaaaay, all shuffled up', {as_user: true}))
            .catch(err => console.log(err))
        }
    },
    ...
    begin: function(message) {
      api_util.getCurrentTourney()
        ...
        .catch(err => {
          console.log(err);
          this.postMessageToChannel(message.channel.name, 'i cant start a tournament that doesnt exist', {as_user: true});
        });

        var startTournament = (tournamentId) => {
          api_util.beginTournament(tournamentId)
            .then(body => {
              this.postMessageToChannel(message.channel.name, 'TIME TO SMAAAAAAAAAASH! \n (i started it)', {as_user: true});
            })
            .catch(err => console.log(err))
        }
    }
