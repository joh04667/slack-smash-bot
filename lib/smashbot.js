...
delete: Delete an open tournament if one exists\`\`\``, {
    ...
    add: function(message) {
        ...
        api_util.getCurrentTourney()
            .then(res => {
                if (!res.length) {
                    this.postMessageToChannel(message.channel.name, `No open tournament right now tho. Open a new one with \`smashbot tournament\` first`, {
                        as_user: true
                    })
                } else {
                    api_util.addPlayers(res[0].tournament.id, message.command.users)
                        .then(done => {
                            ...
                            this.postMessageToChannel(message.channel.name), `Okey dokey, I added ${message.command.users.slice(-1, 0, 'and').toString()}.`
                        })
                ...
                if (!res.length) {
                    ...
                    api_util.getParticipants(res[0].tournament.id)
                        .then(result => {
                            ...
                            var participants = participantArray.length ? "```" + participantArray.join('\n') + "```" : "_none yet_"
                        ...
                        .catch((err, body) => console.log('error getting staatus'))
                }
            })
