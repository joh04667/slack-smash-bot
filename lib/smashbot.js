...
SmashBot.prototype._errorHandle = function(message, error) {
  console.log(error, 'handled by handler');
    ...
    switch (error.name) {
      case 'ApiError':
        if(error.status == 422) {
          message._reply(`Challonge didn't like that. Looks like someone is being added twice. Here's what they said: \n \`\`\`${error.body.errors}\`\`\``)
        } else {
                  ...
                  this._errorHandle(message, new TournamentError({exists: false}));
                    ...
                    api_util.addPlayers(res[0].tournament.id, message.command.users)
                        .then(done => {
                            ...
                            // var longerThanOne = userStr.length > 1
                            //
                            // userStr = userStr.map(s => s.capCase()).join(', ');
                            //
                            // // add an 'and' without an oxford comma, if appropriate
                            // if(longerThanOne) {
                            //   userStr = userStr.replace(/, (\w+)$/i, ' and $1');
                            // }
