...
var api_util = {
    ...
    addPlayers: function(tournamentId, userArray) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            userArray.forEach((s, i) => parameterString += `participants[][name]=${util.capCase(s)}${i < userArray.length - 1 ? '&' : ''}`)
    ...
    removePlayer: function(player) {
        ...
        return new Promise(function(resolve, reject) {
            self.getCurrentTourney()
                .then((res) => {
                    if (!res.length) {
                      ...
                      console.log(res[0].tournament.id);
            ...
            var findPlayer = function(tournamentId, player) {
                ...
                self.getParticipants(tournamentId)
                    .then(playerList => {
                      ...
                      if(found === undefined) {
                        ...
                        return reject(new ApiError(`Error: Participant ${player} was not found`, 422,
                           {errors: [`Error: Couldn't find ${util.capCase(player)} in the current tournament.`]})
                         )
                      }
